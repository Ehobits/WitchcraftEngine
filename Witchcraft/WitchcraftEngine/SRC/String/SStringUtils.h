...
namespace SString
{
	...
	{
		std::wstring result;
		size_t len = MultiByteToWideChar(CP_ACP, 0, str.c_str(), (int)str.size(), NULL, 0);
	...
	{
		std::string result;
		size_t len = WideCharToMultiByte(CP_ACP, 0, wstr.c_str(), (int)wstr.size(), NULL, 0, NULL, NULL);
		if (len <= 0)return result;
