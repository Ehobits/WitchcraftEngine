...
UINT AmbientOcclusion::ComputeScaledDimension(UINT sourceDimension)
{
...
AOConstants AmbientOcclusion::BuildConstants(
...
void AmbientOcclusion::ClearAmbientMapsToNeutral(ID3D12GraphicsCommandList* cmdList)
{
	if (cmdList == nullptr)
		return;
...
void AmbientOcclusion::RecordSsaoPasses(
...
{
	if (cmdList == nullptr || srvDescriptorHeap == nullptr)
		return;
	if (ssaoPipelineState == nullptr || blurPipelineState == nullptr)
		return;
	if (normalDepthSrvHandle.ptr == 0)
		return;
...
void AmbientOcclusion::SetViewports(ID3D12GraphicsCommandList* cmdList)
{
	if (cmdList == nullptr)
		return;
...
void AmbientOcclusion::SetRenderTargets(ID3D12GraphicsCommandList* cmdList)
{
	if (cmdList == nullptr)
		return;
...
void AmbientOcclusion::TransitionAmbientMap0(ID3D12GraphicsCommandList* cmdList, D3D12_RESOURCE_STATES targetState)
{
	TransitionTrackedResourceState(cmdList, mAmbientMap0.Get(), mAmbientMap0State, targetState);
}

void AmbientOcclusion::TransitionAmbientMap1(ID3D12GraphicsCommandList* cmdList, D3D12_RESOURCE_STATES targetState)
{
	TransitionTrackedResourceState(cmdList, mAmbientMap1.Get(), mAmbientMap1State, targetState);
}

void AmbientOcclusion::BlurAmbientMap(
	ID3D12GraphicsCommandList* cmdList,
	ID3D12PipelineState* blurPipelineState,
	bool horzBlur,
	D3D12_GPU_VIRTUAL_ADDRESS aoCBAddress,
...
void AmbientOcclusion::GetOffsetVectors(DirectX::XMFLOAT4 offsets[SsaoSampleCount]) const
{
