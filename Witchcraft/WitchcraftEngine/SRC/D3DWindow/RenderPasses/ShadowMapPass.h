...
public:
	...
	void Initialize(ID3D12Device* device, ID3D12GraphicsCommandList* cmdList, int width, int height);
	...
	// 创建顶点布局和管道
	void CreatePipesAndShaders(ComPtr<ID3DBlob>& vertexShader, ComPtr<ID3DBlob>& pixelShader, D3D12_GRAPHICS_PIPELINE_STATE_DESC basePsoDesc, ComPtr<ID3D12PipelineState>& PipelineState);
